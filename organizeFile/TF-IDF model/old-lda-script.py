...
def train_and_evaluate_model(train_df, val_df, test_df):
    ...
    val_predictions = lda.predict(X_val_tfidf.toarray())
...
if __name__ == "__main__":
    ...
    # new_test_texts = ['干脆别有信访办啊，反正大家都在微博举身份证', '#山东高院工作报告#', '#北京交通管制#北京交通管制真是没有下限了，一个多小时了还不解除。发声平台这么多以后能不能提前通知？不能通知的话能不能让领导快走两步？', '我不喜欢总统', '中国的GDP下降得太低了。我们需要更多的投资', '我最近感觉不舒服。']

    # # Vectorize the new test text data using the loaded vectorizer
    # new_test_tfidf = loaded_vectorizer.transform(new_test_texts)

    # # Make predictions on the new test data
    # new_test_predictions = loaded_model.predict(new_test_tfidf.toarray())
    # new_test_probabilities = loaded_model.predict_proba(new_test_tfidf.toarray())

    # # Output the predictions
    # for text, prediction, probabilities in zip(new_test_texts, new_test_predictions, new_test_probabilities):
